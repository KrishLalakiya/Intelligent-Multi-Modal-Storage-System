...
//     toggleModal(false);
//     updateUploadAreaUI([]); // Reset the upload area UI after upload starts
...
async function uploadFiles() {
    ...
    if (files.length === 0) {
        showError("⚠️ Please select files to upload before clicking the button.");
        console.warn("Upload blocked: No files selected.");
        return;
    }
    // ------------------------------------
    ...
    console.log(`Starting upload process for ${files.length} file(s)...`); // Confirmation log
...
// --- NEW FUNCTION TO UPDATE UPLOAD UI ---
...
function toggleModal(show) {
    ...
    if (!show) {
        fileInput.value = '';
        updateUploadAreaUI([]); // Reset the UI when modal closes
    }
}
...
function initApp() {
    ...
    // Drag and Drop (MODIFIED drop listener)
    ...
    dropArea.addEventListener('drop', e => {
        ...
        updateUploadAreaUI(e.dataTransfer.files); // Update UI
